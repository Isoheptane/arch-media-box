...
\begin{multicols*}{3}
	...
	\begin{minipage}{\abstractwidth}
		\noindent 核准日期：2023年07月13日
		\newline 修改日期：2019年07月06日，2020年10月31日，2021年07月04日，
		\newline 2021年08月14日，2023年06月10日，2023年07月13日
		\newline
		...
		\begin{mdframed}[hidealllines=true, innerbottommargin=.5em, innertopmargin=0pt]
			...
			某些与多媒体相关的软件，特别是允许回放和提供音频、视频操作或类似功能的软件，不被 Arch Linux 包含，这是因为它在世界上的某些地区被认为是非法的。本说明中的信息和意见无意构成法律建议，法律建议可通过咨询律师来获得。
	...
	\begin{itemize}
		\item 仅支持使用 amd64(x86-64) 架构的计算机、服务器和嵌入式设备。
	...
	Arch Linux 有一个对用户和开发者所提交的软件缺陷报告进行归档管理的缺陷跟踪系统，英文缩写。每个软件缺陷报告都被授予一个编号并且被长期跟踪，直到它被标记为已修复。
	...
	\begin{itemize}[leftmargin=*]
		...
		在罕见情况中已观察到，在使用 Windows 进行系统更新时，可能会出现重新启动后 GRUB 引导被破坏从而导致 Arch Linux 无法启动的情况。同时，若在安装过程中将引导信息写入与 Windows 所在的物理磁盘 MBR 内，将导致后者无法正常启动。

		\item 与其他 Linux 发行版的相互作用

		与 Manjaro 混用时，可能导致用户仓库宕机。
	...
	装有 Arch Linux 安装镜像的、兼容 USB 3.0/2.0 协议的大容量存储设备。
...
% 	\medskip
	...
	\begin{tabularx}{\linewidth}{@{}ll@{}}
		...
		图形: & @YJBeetle\\
        ~ & @Isoheptane \\
		GitHub: & isoheptane/arch-media-box\\
	\end{tabularx}
